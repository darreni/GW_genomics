...
makeDetailedChrPlots <- function(pos, 
  ...
  row.height <- 2 / window.height  # height of each row of the figure (Fst row, Dxy row, and pi row)
  ...
  for (j in 1:length(groups.to.plot.WC84_Fst)) {
    ...
    mtext(side=4, text=expression("Mean " * italic('F')[ST]), col=group.colors.WC84_Fst[j], cex=0.7, las=1, line=1.5)
    mtext(side=2, text=expression(italic('F')[ST]), col="black", cex=0.8, las=1, line=2)
  ...
  for (j in 1:length(groups.to.plot.Dxy)) {
    ...
    new.fig.parameter <- TRUE
    plot(x=NULL, y=NULL, xlim=c(start.bp, end.bp), ylim=c(0, 1.2), yaxp=c(0, 1, n=1), mgp=c(2,0.5,0), ylab=NA, xlab=NA, xaxt='n', yaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=1.25, cex.axis=0.75)
    mtext(side=4, text=expression("100 * mean " * italic('D')[xy]), col=group.colors.Dxy[j], cex=0.7, las=1, line=1.5)
    mtext(side=2, text=expression(italic('D')[xy]), col="black", cex=0.8, las=1, line=2)
    axis(side=2, at=c(0,1), tcl=-0.25, las=1, col="black", col.ticks="black", cex=0.75, mgp=c(2,0.5,0))
  ...
  for (j in 1:length(groups.to.plot.pi)) {
    ...
    new.fig.parameter <- TRUE
    plot(x=NULL, y=NULL, xlim=c(start.bp, end.bp), ylim=c(0, 0.6), yaxp=c(0, 0.5, n=1), mgp=c(2,0.5,0), ylab=NA, xlab=NA, xaxt='n', yaxt='n', bty='n', las=1, xaxs="i", tcl=-0.25, xpd=NA, cex=1, cex.lab=1.25, cex.axis=0.75)
    mtext(side=4, text=expression("100 * mean " * italic(pi)), col=group.colors.pi[j], cex=0.7, las=1, line=1.5)
    mtext(side=2, text=expression(italic(pi) * " "), col="black", cex=0.9, las=1, line=2)
...
plotPCA <- function(Fst.filter, Fst.cutoff, groups.to.compare, WC84_Fst, combo.NApass, num_loc_cols, region.text,
  ...
  SNP.included <- SNP_genotypes[SNP_genotypes$group %in% groups.to.plot.PCA,]
  ...
  for (i in 1:length(groups.to.plot.PCA)) {
    selection <- SNP.included$group == groups.to.plot.PCA[i]
...
plotGenotypeByIndividual <- function(groups.to.compare, WC84_Fst.cutoff, missing.fraction.max,
  ...
  SNP.genotypes.subset <- SNP.genotypes[SNP.genotypes$group %in% plot.groups,]
  if (option == 2) {
  ...
  if (plot.along.chromosome.type == 2 | plot.along.chromosome.type == 3) {
    ...
    for (i in 1:num.inds) {
      ...
      polygon(group.color.box.x.right, y+group.color.box.y, border=NA, col=plot.group.colors[which(plot.groups==sorted.SNP.genotypes.subset.NApass$group[y])])
      polygon(group.color.box.x.left, y+group.color.box.y, border=NA, col=plot.group.colors[which(plot.groups==sorted.SNP.genotypes.subset.NApass$group[y])])
    }
...
plots3Fst_Dxy <- function(comp1, comp2, comp3,
  ...
  # Add cubic spline:
  lines(smooth.spline(x=Fst.vector1, y=Dxy.vector1, spar=1), lty = 1, col = alpha('green3', 0.75), lwd=2)  # 0.8 is transparency
  # second plot:
  ...
  lines(smooth.spline(x=Fst.vector2, y=Dxy.vector2, spar=1), lty = 1, col = alpha("orange", 0.75), lwd=2)
  # third plot:
  ...
  lines(smooth.spline(x=Fst.vector1, y=Dxy.vector1, spar=1), lty = 1, col = alpha("purple", 0.75), lwd=2)
...
plotMeanPi_MeanPi <- function(group1A, group1B, group2A, group2B,
  ...
  quartz(title=paste0("Scatterplot of mean windowed pi, based on compiled chromosomes, between ", group1A,"_", group1B, " vs ", group2A,"_", group2B, sep=""), width=8, height=8)
...
plotDxy_MeanPi <- function(group1, group2, cor.method,
  ...
  quartz(title=paste0("Scatterplot of windowed mean pi vs. Dxy between ", group1,"_", group2, sep=""), width=5, height=4.75)
  plot(x=Dxy, y=mean_pi, col=colors, xlim=c(0,max(c(max(Dxy)*1.04,max.x))), ylim=c(0, max(c(max(mean_pi)*1.04,max.y))),
       xaxp = c(0, max.x, 2), yaxp=c(0, max.y, 2), pch=16, cex=0.25, cex.axis=0.9, tcl=-0.5, xlab=NA, ylab=NA, mgp=c(3,0.5,0))
  lines(c(0,1), c(0,1))
  label.size <- 1.1
  if (color_bar == T) {
    colorbar.plot(x=0, y=1.04*max.y, seq(1,100,by=1), col = color.palette, strip.width=0.05, strip.length=0.5, horizontal=T, adj.x = 0, adj.y = 0)
    text(x=max.x/4, y=1.07*max.y, bquote(italic('F')[ST]* " scale"), pos=3, cex=0.8)
    text(x=max.x/100, y=1.05*max.y, "0", pos=1, cex=0.7)
    text(x=max.x/2.1, y=1.05*max.y, "1", pos=1, cex=0.7)
  }
...
plotMeanPi_Dxy.autosome_Z <- function(group1, group2,
  ...
  xlimits <- c(0, round(max(c(autosome.Dxy,Z.Dxy))*1.04, digits=3))  # rounds to 3 decimal places
  ylimits <- c(0, round(max(c(autosome.mean_pi,Z.mean_pi))*1.04, digits=3))
